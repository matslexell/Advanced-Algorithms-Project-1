...
public class Main {
	...
	public static void printCsv(Map<BigInteger, Integer> factors, BigInteger number){
...
//		List<>factors.keySet();
	...
	private static boolean validateFactors(Map<BigInteger, Integer> factors,
			BigInteger product) {
	...
	private static void testFactorProductOf(
			Map<BigInteger, Integer> primeFactors) {
	...
	public static BigInteger getFactor(BigInteger n)
		...
		try {
			...
			smallerFactor = PollardRho.pollardRhoInner(n,
					BigInteger.valueOf(2), x -> g.g(x));
	...
	/**
	 * Return the prime factors of given number. Factors are returned in a map
	 * with factors as keys and the factor-powers as values. For instance
	 * getPrimeFactors(12) == {2:2, 3:1} since 12 = 2^2 * 3^1
	 */
	...
	public static void testGauss() {
		...
		for (ArrayList<Integer> solution : solutions) {
			Printer.MAIN.println("Product of "
					+ Arrays.toString(solution.stream()
							.mapToInt(i -> cols.get(i).getNumber().intValue())
							.toArray()) + " is a square");
		}
	}
